<h1 align="center">🔧 SOS Motors – Assistência Automotiva</h1>

<p align="center">
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-ES6-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/PRs-Welcome-brightgreen?style=for-the-badge"/>
</p>

<p align="center">
  Sistema criado para oferecer uma solução rápida, prática e eficiente para problemas automotivos. <br>
  O <strong>SOS Motors</strong> inclui cadastro de veículos, mecânicos, painel informativo e páginas institucionais.
</p>

---

# 🔗 Acesse a versão final aqui:

https://sos-motors.vercel.app/

Ou escaneie o QR Code abaixo:

  <img width="448" height="432" alt="image" src="https://github.com/user-attachments/assets/5af58904-5da8-42b5-9238-14f94c929880" />

<p align="center">
  <!-- Adicionar imagem do QR Code -->
</p>

---

# 🖥 Demonstração das Telas

### 🏁 Tela Inicial  
Tela introdutória com acesso rápido às funcionalidades principais.

<p align="center">
  <img width="1337" height="630" alt="image" src="https://github.com/user-attachments/assets/1fd59c94-6398-4d55-a38f-f9e97389b000" />

</p>

---

### ℹ Sobre  
Página apresentando informações sobre a proposta e missão do SOS Motors.

<p align="center">
  <img width="1355" height="641" alt="image" src="https://github.com/user-attachments/assets/051b5d93-7b2e-4840-b0bd-50870056b337" />

</p>

---

### 📊 Painel  
Visão geral do sistema, exibindo informações essenciais de forma clara e objetiva.

<p align="center">
  <!-- Adicionar imagem -->
  <img width="1338" height="633" alt="image" src="https://github.com/user-attachments/assets/64ef7767-cffe-4c77-905b-3ec95b41a98f" />

</p>

---

### 🚘 Cadastro de Placa  
Formulário simples para cadastrar veículos e suas respectivas placas.

<p align="center">
  <!-- Adicionar imagem -->
  <img width="1335" height="635" alt="image" src="https://github.com/user-attachments/assets/69c4d3f8-770e-4d51-8e26-92defe3ee370" />

</p>

---

### 🧰 Mecânicos  
Tela dedicada ao encontro de melhores mecânicos próximos a você.

<p align="center">
  <!-- Adicionar imagem -->
  <img width="1346" height="641" alt="image" src="https://github.com/user-attachments/assets/a47b5aa6-afc2-4d19-94b3-ed354ce76873" />

</p>

---

# 💻 Tecnologias Utilizadas

<p align="left">
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-ES6-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
</p>

| Tecnologia | Função |
|-----------|--------|
| *HTML5* | Estrutura do sistema |
| *CSS3* | Estilização e interface visual |
| *JavaScript* | Funcionalidade e interações |

---

# 🧠 Sobre o Projeto

O *SOS Motors* foi desenvolvido como um projeto acadêmico com foco em agilidade, clareza e experiência do usuário.  
Reúne páginas essenciais para um sistema de assistência automotiva moderno e funcional.

<p align="center">⭐ Se este repositório foi útil, deixe uma estrela ⭐
